...
func (b *BalanceThresholdEventHandler) handleRule(ctx context.Context, balSnapshot snapshot.SnapshotEvent, rule notification.Rule) error {
	...
	if err != nil {
		if err == ErrNoBalanceAvailable {
			// This can only happen if the user gets an entitlement, receives a grant, then the grant gets
			// voided. Also at least one of the thresholds must be percentage based.

			if *balSnapshot.Value.Balance+*balSnapshot.Value.Usage != 0 {
				// Let's create a new event so that we are not having issues with the next calculation
				return b.createEvent(ctx, createEventInput)
			}

			b.Logger.Warn("no balance available for last event, skipping event creation", "last_event_id", lastEvent.ID)
			return nil
		}
...
func getBalanceThreshold(threshold notification.BalanceThreshold, eValue api.EntitlementValue) (balanceThreshold, error) {
	...
	case api.PERCENT:
		...
		// - Then the granting is done as a seperate step
		//
